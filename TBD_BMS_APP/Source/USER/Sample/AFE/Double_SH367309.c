...
t_SH367309_EEPRom gSHAfeEepRead_H;                        //SH367309配置EEP结构体
t_SH367309_Register gSHAFEReg;                          //SH367309寄存器结构体
t_SH367309_Register gSHAFEReg_H;                          //SH367309寄存器结构体
...
//=============================================================================================
//函数名称	: static u8 SH367309CalcCRC(u8 *datadataptr, u8 len)
...
//=============================================================================
//函数名称: s8 BQ769X0_ChangADtoTemp(u8 ishighafe,u8 hibyte, u8 lobyte)
//输入参数: TempAD采样AD值
//输出参数: 转化后的温度值
//功能描述: 温度AD值转化为实际温度值
//注意事项:
	...
	switch(sstep)
	{
		...
		case 4:
           //SCONF1
            ...
            gSHAfeEep.SCONF1.SCONF1Bit.CN = SH367309_EEP_CELL10;            //10节电芯应用
	...
	switch(sstep)
	{
		...
		case 4:
           //SCONF1
			...
			gSHAfeEep.SCONF1.SCONF1Bit.CN = SH367309_EEP_CELL7;            //7节电芯应用
        ...
        case 0xaa:
            if(BSPIICChannalStateGet(gSHAFEData.afechn_h) <= 0)
            {
                ...
                else
                {
                    ...
                    else if(5 == slaststep && i < (sizeof(t_SH367309_EEPRom) - 1))  //配置EEP
                    {
                        //烧写完EEP一个字节后，至少延时35ms
                        i++;
                        sstep = slaststep;
...
static void SH367309NormalTask(void)
{
    ...
    if(gSHAFECtrl.eventctrl & SH367309_EVE_CTRL_SMP)
    {
        ...
        else
        {
            ...
            //高低端均衡方案
            //不打开充放电MOS: 不动作；
            //打开充电或放电MOS: BL_L_EN、BL_H_EN设置为高电平
            //同时打开充放电MOS:BL_L_EN设置为低电平，BL_H_EN设置为高电平
        ...
        {
            if(index + 6 > sizeof(gSHAFEReg_H)) //代表是最后一帧
    ...
    if(gSHAFECtrl.eventctrl & SH367309_EVE_CTRL_SLP)
    {
		...
		#if defined(USE_B22_IM_PCBA)	
        SH367309_H_SHIP_ON;   /* 仓运模式开启 */
		#endif
        SH367309_IIC_DISABLE; /* 关闭IIC */
        gSHAFEData.runstatus = e_SH_AFE_Sta_Disable;
        SH367309EventFlagClear(SH367309_EVE_CTRL_SLP);
    }
	...
	if(gSHAFEData.iicerr >= 10)	//出现断线故障
	{
		gSHAFEData.runstatus = e_SH_AFE_Sta_Offline;
	}
